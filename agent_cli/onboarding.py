...
"""
    ...
    def _onboard_ollama(self, info: Dict) -> Optional[Dict[str, str]]:
        ...
        color = info["color"]
        self.console.print(
            f"[{color}]Ollama runs locally - no API key needed! {info['icon']}[/{color}]\n"
        )
        ...
        if models:
            ...
            default_model = Prompt.ask("Model name", default="llama3.3", console=self.console)
        ...
        keep_alive = Prompt.ask("Keep-alive (minutes)", default="5", console=self.console)
    ...
    def _onboard_api_provider(self, provider: str, info: Dict) -> Optional[Dict[str, str]]:
        ...
        has_key = Confirm.ask("Do you have an API key?", default=False, console=self.console)
