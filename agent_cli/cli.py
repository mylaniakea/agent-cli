...
def chat(provider, model, interactive, stream, prompt):
    ...
    if interactive:
        ...
        if not provider or not model:
            ...
            if session_state:
                ui.print_info(f"Last session used: {session_state.get('provider')} / {session_state.get('model')}")
                ui.print_info("Use --interactive to continue with last session, or specify --provider and --model.")
            sys.exit(1)
    ...
    if not ModelFactory.validate_model(current_provider, current_model):
        ui.print_warning(f"Model '{current_model}' not found in metadata for provider '{current_provider}'.")
    ...
    def create_agent():
        ...
        return AgentFactory.create(current_provider, current_model, config, system_prompt=current_system_prompt)
    
    if interactive:
        ui.print_welcome()
        ui.print_info(f"Using [bold]{current_provider}[/bold] with model [bold]{current_model}[/bold]")
        if current_stream:
        ...
        while True:
            try:
                ...
                if user_input.startswith("/"):
                    ...
                    if cmd == "/compress":
                        ...
                        if not bd_path:
                            ...
                            if brew_path:
                                try:
                                    response = ui.interactive_session.session.prompt("Install 'bd' via Homebrew? (y/n) > ")
                                    if response.lower().startswith('y'):
                                        ...
                                        if bd_path:
                                            ...
                                            ui.print_error("Installation appeared to succeed but 'bd' is still not found.")
                            ...
                            if not bd_path:
                                ...
                                ui.print_info("  -or-")
                                ui.print_info("  curl -fsSL https://raw.githubusercontent.com/steveyegge/beads/main/scripts/install.sh | bash")
                                continue
                        ...
                        try:
                            ...
                            ui.print_warning(f"Beads returned error (is it initialized?): {e.stderr.strip() if e.stderr else e.stdout.strip()}")
                        ...
                        if args:
                             ...
                             ui.print_info(f"Current keep-alive: {getattr(agent, 'keep_alive', 'N/A')}")
                    ...
                    if handled:
                        ...
                        new_system_prompt = command_context.get(CONTEXT_KEY_SYSTEM_PROMPT, current_system_prompt)
                        ...
                        elif (new_model != command_context.get(CONTEXT_KEY_MODEL, current_model) or 
                              new_provider != command_context.get(CONTEXT_KEY_PROVIDER, current_provider) or
                              new_system_prompt != command_context.get(CONTEXT_KEY_SYSTEM_PROMPT, current_system_prompt)): # Check if these differ from what they were BEFORE command
                        ...
                        ui.print_error(f"Unknown command: {user_input}. Type /help for available commands.")
                    continue
                ...
                ui.interactive_session.update_status(current_provider, current_model, connected=True) # Assume connected until error
                ...
                ui.interactive_session.update_status(current_provider, current_model, connected=False)
                ui.print_error(f"{e}")
    else:
        if not prompt:
        ...
        try:
            ...
            if stream:
                for token in agent.stream(processed_prompt, history):
                    print(token, end="", flush=True) # Keep raw print for pipe-ability in non-interactive? Or use ui? 
